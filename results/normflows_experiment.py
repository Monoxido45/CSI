...
import os
from CSI.utils import obtain_quantiles_saved_tune_nflow, CPU_Unpickler
...
def compute_MAE_N_B(
    ...
    mae_array = np.zeros((n_replica, 1))

    # just one column: nflow
    if using_cpu:
        with open(
    ...
    if not completing:
        for i in tqdm(range(n_replica), desc="Computing MAE for normflows: "):
            quantiles_dict = obtain_quantiles_saved_tune_nflow(
                kind=kind,
                score=score,
            ...
            err_data = np.zeros((theta_grid_eval.shape[0], 1))
            l = 0
            for theta in theta_grid_eval:
                ...
                nflow_cover = np.mean(stat <= quantiles_dict["nflow"][l])

                # appending the errors
                err_nflow = np.abs(nflow_cover - (1 - alpha))
            ...
            stats_data.to_csv(save_path + f"/MAE_data_nflow_{N}_{B}.csv")
    else:
        torch.manual_seed(completing_seed)
        torch.cuda.manual_seed(completing_seed)
        stats_data = pd.read_csv(save_path + f"/MAE_data_nflow_{N}_{B}.csv")

        mae_array = stats_data.iloc[:, 1:2].to_numpy()

        for i in tqdm(
            range(completing_n - 1, n_replica), desc="Computing MAE for each nflow: "
        ):
            quantiles_dict = obtain_quantiles_saved_tune_nflow(
                kind=kind,
                score=score,
