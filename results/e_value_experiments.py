...
def obtain_quantiles(
    ...
    thetas_sim, model_lambdas = sim_obj.FBST_sample(B=B, N=N)
    ...
    for theta in theta_valid:
        lambda_valid[i, :] = sim_obj.FBST_sim_lambda(theta, B=N_lambda, N=N)
        i += 1
    ...
    # FBST quantile
    alpha_quantiles = np.repeat(1 - alpha, thetas.shape[0])
    ...
    for N_fixed in N:
        for B_fixed in B:
            ...
            for it in range(0, n_it):
                ...
                err_data = np.zeros((thetas.shape[0], 6))
                l = 0
                for theta in thetas:
                    ...
                    stat = sim_obj.FBST_sim_lambda(theta=theta, B=n, N=N_fixed)
                    ...
                    err_alpha = np.mean(stat <= quantiles_dict["alpha"][l])
                    ...
                    err_alpha = np.abs(err_alpha - (1 - alpha))
            ...
            stats_data.to_csv(
                original_path
                + folder_path
                + "/e_value_{}_stats_data.csv".format(kind_model)
            )
            ...
            np.savez(
                original_path
                + folder_path
                + "/e_value_{}_rng_simulate_list".format(kind_model),
                rng_simulate_array,
                allow_pickle=True,
            )
            ...
            np.savez(
                original_path
                + folder_path
                + "/e_value_{}_rng_test_list".format(kind_model),
                rng_test_array,
                allow_pickle=True,
            )
