...
def obtain_quantiles(
    ...
    thetas_sim, model_lambdas = sim_obj.KS_sample(B=B, N=N)
    ...
    for theta in theta_valid:
        lambda_valid[i, :] = sim_obj.KS_sim_lambda(theta, B=N_lambda, N=N)
        i += 1
        ...
        for B_fixed in B:
            ...
            for it in range(0, n_it):
                ...
                for theta in thetas:
                    ...
                    stat = sim_obj.KS_sim_lambda(theta=theta, B=n, N=N_fixed)
            ...
            stats_data.to_csv(
                original_path + folder_path + "/KS_{}_stats_data.csv".format(kind_model)
            )
            ...
            np.savez(
                original_path
                + folder_path
                + "/KS_{}_rng_simulate_list".format(kind_model),
                rng_simulate_array,
                allow_pickle=True,
            )
            ...
            np.savez(
                original_path + folder_path + "/KS_{}_rng_test_list".format(kind_model),
                rng_test_array,
                allow_pickle=True,
            )
...
if __name__ == "__main__":
    print("We will now compute all MAE statistics for the KS statistic")
